...
export abstract class ApiService {
     ...
     protected get<T>(url: string): Observable<T> {
          return this.http.get<T>(url).pipe(
               ...
               catchError((error: HttpErrorResponse) => {
                    this.notificationService.error(`${COMMON_MESSAGES.ERROR_OCCURED} ${error.message}`);
     ...
     protected post<T>(url: string, requestData: unknown): Observable<T> {
          return this.http.post<T>(url, requestData).pipe(
               shareReplay(1),
               catchError((error: HttpErrorResponse) => {
                    this.notificationService.errorSave(`${COMMON_MESSAGES.ERROR_OCCURED} ${error.message}`);
                    return throwError(() => error);
               })
          );
     }
